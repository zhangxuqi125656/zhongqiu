...
function loadQuestions() {
    ...
    return fetch('questions.json')
        .then(response => {
            console.log('题库文件响应状态:', response.status);
            if (!response.ok) {
                throw new Error('题库文件加载失败: ' + response.statusText);
            }
            return response.json();
        })
        .then(data => {
            console.log('题库数据加载成功，题目数量:', Object.keys(data).length);
            console.log('题库前5题序号:', Object.keys(data).slice(0, 5).join(', '));
...
function getQuestion(questionNumber) {
    console.log('尝试获取题目，序号:', questionNumber);
    console.log('题库中可用的题目序号:', Object.keys(questionDatabase).join(', '));
    const question = questionDatabase[questionNumber] || null;
    console.log('找到的题目:', question);
    return question;
}
...
confirmNumberBtn.addEventListener('click', function() {
    ...
    console.log('用户输入的题目序号:', inputNumber);
    console.log('输入序号的长度:', inputNumber.length);
    console.log('输入序号的类型:', typeof inputNumber);
    ...
    if (!question) {
        console.error('未找到题目，序号:', inputNumber);
